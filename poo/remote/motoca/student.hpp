...
public:
    // criar o construtor
    Motoca(int potencia=1, int time=0, string pessoa=nullptr) : potencia(potencia), time(time), pessoa(pessoa){}
...
/*Crie o método `inserir(pessoa: Pessoa): bool` que permite inserir uma pessoa na motoca.
  ...
  - Crie o método `toString()` para mostrar o estado da motoca.*/
    ...
    bool inserir(Pessoa* p) {
        ...
        pessoa = p;
        cout << "Pessoa inserida na motoca: " << pessoa->toString() << endl;
        return true;
    }
    Pessoa* remover() {
        if (pessoa == nullptr) {
        ...
        Pessoa* p = pessoa;
        pessoa = nullptr;
        cout << "Pessoa removida da motoca: " << p->toString() << endl;
        return p;
    }
    ...
    void buyTime(int t) {
        time += t;
        cout << "Tempo comprado: " << time << " minutos." << endl;
    }
    ...
    void drive(int t) {
        ...
        time -= t;
        cout << "Dirigido por " << t << " minutos. Tempo restante: " << time << " minutos." << endl;
    }
  ...
  - Retorne a buzina.*/
    ...
    string toString()  {
        string estado = "Potência: " + to_string(potencia) + ", Tempo: " + to_string(time) + " minutos";
        if (pessoa != nullptr) {
            estado += ", Pessoa: " + pessoa->toString();
        } else {
            estado += ", Pessoa: Nenhuma";
        }
        return estado;
    }
...
public:
    ...
    void init(int power = 1, int time, string pessoa) {
        (void) power;
        motoca = Motoca(power, time, pessoa);
    }
